  ...
  describe("validateUserOp", () => {
    ...
    context("when the sender is the entry point", () => {
      ...
      context("when a signature is given", () => {
        context("when the signature can be decoded", () => {
          ...
          context("when signed by the owner", () => {
              ...
              context("when the given nonce is correct", () => {
                context("when the op is signed by the owner", () => {
                  ...
                  const itIncreasesTheWalletNonce = (prefund: BigNumberish) => {
                    it("increases the wallet nonce", async () => {
                      ...
                      await wallet.validateUserOp(op, requestId, prefund, {
                        from,
                      });
                  ...
                  const itIgnoresAnyOtherOpData = (prefund: BigNumberish) => {
                    it("ignores any other op data", async () => {
                      ...
                      requestId = await wallet.getRequestId(op);
                      await expect(
                        wallet.validateUserOp(op, requestId, prefund, { from })
                      ).not.to.be.reverted;
                    });
                  };
                  ...
                  context("when a prefund is required", () => {
                    ...
                    context("when the wallet has some funds", () => {
                      ...
                      context("when the entry point reverts", () => {
                        ...
                        it("reverts", async () => {
                          await expect(
                            wallet.validateUserOp(op, requestId, prefund, {
                              from,
                            })
                          ).to.be.revertedWith("ENTRY_POINT_RECEIVE_FAILED");
                        });
                      });
                    });
                ...
                context("when the op is signed by someone else", () => {
                  beforeEach("sign op", async () => {
                    ...
                    op.signature = encodeSignatures(type, {
                  ...
                  it("reverts", async () => {
                    await expect(
                      wallet.validateUserOp(op, requestId, { from })
            ...
            context(
              "when the signed request ID does not match the one given",
              () => {
                ...
                it("reverts", async () => {
                  await expect(
                    wallet.validateUserOp(op, requestId, { from })
                  ).to.be.revertedWith("ACL: Invalid owner sig");
                });
              }
            );
          });
          ...
          context("when signed by the guardians", () => {
            ...
            context("when the wallet allows some guardians", () => {
              context("when some calldata is given", () => {
                context("when the guardian is transferring ownership", () => {
                  ...
                  context(
                    "when the signed request ID matches the one given",
                    () => {
                      beforeEach("compute request ID", async () => {
                        requestId = await wallet.getRequestId(op);
                      });
                      ...
                      context("when the op is signed by a guardian", () => {
                        ...
                        context(
                          "when the amount of signatures is below the min required",
                          () => {
                            beforeEach("add guardians", async () => {
                              const [guardian2, guardian3, guardian4] =
                                await getSigners(3, 4);
                      ...
                      context("when the op is signed by someone else", () => {
                        ...
                        it("reverts", async () => {
                          await expect(
                            wallet.validateUserOp(op, requestId, { from })
                          ).to.be.revertedWith("ACL: Signer not a guardian");
                        });
                      });
                    }
                  );
                  ...
                  context(
                    ...
                    () => {
                      ...
                      beforeEach("sign op", async () => {
                        op.signature = await wallet.signRequestIdWithGuardians(
                          op
                        );
                      });
                      ...
                      it("reverts", async () => {
                        await expect(
                          wallet.validateUserOp(op, requestId, { from })
                        ).to.be.revertedWith("ACL: Invalid guardian sig");
                      });
                    }
                  );
                });
                ...
                context("when the guardian is approving tokens", () => {
                  ...
                  context(
                    "when the guardian is approving tokens to the paymaster",
                    () => {
                      beforeEach("set calldata", async () => {
                        op.paymaster = wallet.address;
                        op.callData = await encodeWalletExecute(
                          token,
                          await encodeTokenApproval(wallet, fp(10))
                        );
                      });

                      context(
                        "when the token being approved matches the token fee used by the paymaster",
                        () => {
                          beforeEach("set calldata", async () => {
                            op.paymasterData = encodePaymasterData(
                              { token, feed: token, mode: 0, fee: 0 },
                              "0x"
                            );
                          });
                          ...
                          context(
                            ...
                            () => {
                              ...
                              context(
                                "when the op is signed by a guardian",
                                () => {
                                  beforeEach("sign op", async () => {
                                    op.signature =
                                      await wallet.signRequestIdWithGuardians(
                                  ...
                                  context(
                                    ...
                                    () => {
                                      it("increases the wallet nonce", async () => {
                                        ...
                                        expect(
                                          await wallet.nonce()
                                        ).to.be.equal(previousNonce.add(1));
                                      });
                                      ...
                                      it("ignores any other op data", async () => {
                                        ...
                                        requestId = await wallet.getRequestId(
                                          op
                                        );
                                        await expect(
                                          wallet.validateUserOp(op, requestId, {
                                      ...
                                      it("does not transfer any funds to the entry point", async () => {
                                        ...
                                        expect(
                                          currentWalletBalance
                                        ).to.be.equal(previousWalletBalance);
                              ...
                              context(
                                "when the op is signed by someone else",
                                () => {
                                  beforeEach("sign op", async () => {
                                    const signature = await other.signMessage(
                                      ethers.utils.arrayify(requestId)
                      ...
                      context(
                        "when the token being approved does not match the token fee used by the paymaster",
                        () => {
                          beforeEach("set paymaster data", async () => {
                            const anotherToken = await deploy("TokenMock", [
                              "DAI",
                              18,
                            ]);
                            op.paymasterData = encodePaymasterData(
                          ...
                          it("reverts", async () => {
                            await expect(
                              wallet.validateUserOp(op, requestId, { from })
                            ).to.be.revertedWith("Invalid guardian action");
                          });
                        }
                      );
                    }
                  ...
                  context(
                    "when the guardian is approving tokens to someone else",
                    () => {
                      beforeEach("set calldata", async () => {
                        op.callData = await encodeWalletExecute(
                          token,
                          await encodeTokenApproval(guardian, fp(10))
                        );
                      });
                ...
                context(
                  "when the guardian is trying to make another call",
                  () => {
                    beforeEach("set calldata", async () => {
            ...
            context("when there are no guardians in the wallet", () => {
              ...
              it("reverts", async () => {
                await expect(
                  wallet.validateUserOp(op, requestId, { from })
                ).to.be.revertedWith("Wallet: No guardians allowed");
              });
            });
          });
        });
        ...
        context("when the signature cannot be decoded", () => {
          ...
          it("reverts", async () => {
            await expect(wallet.validateUserOp(op, ZERO_BYTES32, 0, { from }))
              .to.be.reverted;
          });
        });
      });
      ...
      context("when no signature is given", () => {
        it("reverts", async () => {
          await expect(wallet.validateUserOp(op, ZERO_BYTES32, 0, { from })).to
            .be.reverted;
        });
      });
    });
    ...
    context("when the sender is not the entry point", () => {
      ...
      it("reverts", async () => {
        await expect(
          wallet.validateUserOp(op, ZERO_BYTES32, 0, { from })
  ...
  describe("executeUserOp", () => {
    context("when the sender is the entry point", () => {
      ...
      context("when the call does not revert", () => {
        ...
        context("when the call does not require ETH", () => {
          it("executes the call", async () => {
            const receipt = await wallet.executeUserOp(
              mock,
              await encodeCounterIncrement(),
              { from }
            );
          ...
          it("does not affect the wallet balance", async () => {
            ...
            await wallet.executeUserOp(mock, await encodeCounterIncrement(), {
              from,
            });
        ...
        context("when the call is only to transfer ETH", () => {
          ...
          context("when the wallet does not have funds", () => {
            it("reverts", async () => {
              await expect(
                wallet.executeUserOp(guardian, "0x", value, { from })
              ).to.be.reverted;
            });
          });
        });
      ...
      context("when the call reverts", () => {
        ...
        it("reverts", async () => {
          await expect(
            wallet.executeUserOp(mock, await encodeReverterFail(), { from })
    ...
    context("when the sender is not the entry point", () => {
      ...
      it("reverts", async () => {
        await expect(
          wallet.executeUserOp(ZERO_ADDRESS, "0x", { from })
        ).to.be.revertedWith("ACL: sender not allowed");
      });
    });
  });
    ...
    context("when the sender is the entry point", () => {
      ...
      context("when the new implementation is UUPS-compliant", () => {
        ...
        it("upgrades to the new implementation", async () => {
          ...
          expect(await wallet.getCurrentImplementation()).to.be.equal(
      ...
      context("when the new implementation is not UUPS-compliant", () => {
        ...
        it("reverts", async () => {
          await expect(
            wallet.upgradeTo(newImplementation, { from })
          ).to.be.revertedWith(
